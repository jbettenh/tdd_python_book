...
class NewVisitorTest(LiveServerTestCase):
    ...
    def wait_for_row_in_list_table(self, row_text):
        ...
        while True:
            try:
                table = self.browser.find_elements(webdriver.common.By.ID, 'id_list_table')
                rows = table.find_elements_by_tag_name('tr')
                self.assertIn(row_text, [row.text for row in rows])
                return
            except (AssertionError, WebDriverException) as e:
                if time.time() - start_time > MAX_WAIT:
                    raise time.sleep(0.5)
    ...
    def test_can_start_a_list_for_one_user(self):
        ...
        inputbox = self.browser.find_elements(webdriver.common.By.ID, 'id_new_item')
    ...
    def test_multiple_users_can_start_lists_at_different_urls(self):
        ...
        self.browser.get(self.live_server_url)
        page_text = self.browser.find_elements(webdriver.common.By.TAG_NAME, 'body').text
